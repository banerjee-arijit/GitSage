    ...
    return parts.map((part, idx) => {
      ...
      if (part.startsWith("`") && part.endsWith("`")) {
        return (
          <code
            key={idx}
            className="px-1.5 py-0.5 rounded bg-white/10 text-[#1c6cc1] font-mono text-xs border border-white/10 break-all"
          >
            {part.slice(1, -1)}
          </code>
        );
            ...
            >
              <div className="flex items-center justify-between px-4 py-2 bg-white/5 border-b border-white/10 text-neutral-400">
                <div className="flex items-center gap-2">
                  <Code2 className="w-3.5 h-3.5 text-[#1c6cc1]" />
              ...
              if (matchOrdered) {
                return (
                  <div key={lIdx} className="flex items-start gap-2.5 pl-2 py-0.5">
                    <span className="shrink-0 w-5 h-5 rounded-full bg-white/10 text-[#1c6cc1] font-mono text-xs flex items-center justify-center font-bold">
                      {matchOrdered[1]}
                    </span>
              ...
              if (trimmed.startsWith("* ") || trimmed.startsWith("- ")) {
                return (
                  <div key={lIdx} className="flex items-start gap-2.5 pl-3 py-0.5">
                    <span className="shrink-0 w-1.5 h-1.5 rounded-full bg-[#1c6cc1] mt-2" />
